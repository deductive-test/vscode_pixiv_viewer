// このファイルは VS Code 本体同梱の vscode.d.ts をコピーしたものです（npm の @types/vscode と同一内容）。
// 出所: <VS Code インストール先>/resources/app/out/vscode-dts/vscode.d.ts （Microsoft, MIT License）
// 用途: npm を使わずに 'vscode' モジュールの型を jsconfig.json + JSDoc で解決するための vendoring。
// VS Code を更新した際は、必要に応じて同じ場所から再コピーすること。
