...
**`git clone` での取得を推奨します**（`.gitattributes` により改行コードが正しく展開されます）。`package.json` がある階層がルートです。

> ⚠️ Windows で取得したフォルダをそのまま Mac にコピーして使うと、改行コードが CRLF のまま混入し、Mac で `build-vsix.sh` が動きません（下の「よくある不具合」参照）。
> **Mac では Mac 上で `git clone` してください。**

### 2. VSIX を作る

ビルド用スクリプトは **OS 専用に分かれています**。自分の OS のファイルだけを使ってください（どちらも同じ内容の VSIX を生成します）。

| OS | 使うファイル | コマンド |
|----|--------------|----------|
| macOS / Linux | `build-vsix.sh`（Bash ＋ `zip`） | `bash build-vsix.sh` |
| Windows | `build-vsix.ps1`（PowerShell ＋ 同梱 `tar.exe`。Bash 不要） | `powershell -ExecutionPolicy Bypass -File .\build-vsix.ps1` |
...
> **よくある不具合（macOS）**: `bash build-vsix.sh` 実行時に
> `invalid option name: set: pipefail` や `command not found` が出る場合、
> スクリプトの**改行コードが CRLF**になっています（Windows 経由でコピーしたときに起こる）。
> 対処は次のどちらか。
>
> - 推奨: Mac 上で改めて `git clone` し直す（`.gitattributes` により LF で展開されます）。
> - その場で直す: `perl -pi -e 's/\r$//' build-vsix.sh`
